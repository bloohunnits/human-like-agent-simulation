...
As far as we can find, nobody has published this combination. Graph memory systems (HippoRAG, Zep, A-Mem, Mem0-g) have no decay or strength. Decay systems (Hou et al., MemoryBank, Generative Agents) have no relations. ACT-R theory comes closest with its associative term, but that spreads one hop from the current focus of attention, its strengths are static in modern practice, and the LLM implementation of ACT-R reduced it to plain query similarity. Memory-to-memory edges that strengthen, fade, and carry multi-hop recall are the open slot. The one adjacent attempt bolted a forgetting score onto a graph memory as pruning only, and its results cut both ways: the pruning was harmless (deleting a tenth of the store left accuracy unchanged), while the graph itself lost to flat vector search. Forgetting wasn't the problem, the graph was. Whether a graph whose traversal is shaped by strength and decay behaves differently is genuinely untested, and the flat baseline stays the one to beat. Details in [docs/RESEARCH.md](docs/RESEARCH.md).
