...
1. **Environment.** TerraLingua is the most interesting (LLM-authored artifacts, birth and death) but has no memory interface. Maya is instrumented and ours. Concordia is the maintained middle. Lean: Maya first, TerraLingua for the headline runs. Needs a hands-on spike in the TerraLingua codebase to cost the integration.
2. **Formulation: settled into an experiment.** Instead of picking between Hou et al.'s exponential-with-reset and ACT-R's power-law-over-history, the decay kernel is swappable and we run both, alone and with the graph (a 2x2 grid). The reset behavior is suspect on its own terms: one mention of an old memory shouldn't rejuvenate it wholesale, and ACT-R's per-retrieval fading terms match the human pattern (brief priming bump, then back near baseline). The reminiscence probe that separates them behaviorally is in [DESIGN.md](DESIGN.md). Spreading along edges stays ACT-R-flavored either way.
...
8. **Does the graph pay for itself?** The one prior graph-plus-forgetting system lost to flat vectors. Our bet is that strength-weighted spreading changes the outcome. The flat baseline stays a first-class competitor and we report honestly if it wins.
