...
`r` is cosine similarity between current context and memory (relevance). `t` is elapsed time in seconds. `g_n` is the consolidation gradient (strength): `g_n = g_{n−1} + S(t)` with `g_0 = 1` and `S(t) = (1−e^(−t))/(1+e^(−t))`. Each recall flattens the memory's own forgetting curve, and spaced recalls add more strength than massed ones (the spacing effect, which the paper cites Roediger and Karpicke for). Validated in a six-participant companion-agent study against a Generative Agents baseline.
